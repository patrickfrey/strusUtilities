...
struct MetaDataComparison
{
	...
	void translate( QueryInterface& query) const
	{
		...
		if (!numval.initFromString( term->value().c_str())) throw strus::runtime_error(_TXT("metadata value not convertible to numeric value"));
	...
	for (; ii != ie; ++ii)
	{
		switch (ii->opCode())
		{
			case analyzer::QueryTermExpression::Instruction::Term:
			{
				...
				{
					throw strus::runtime_error(_TXT("internal: metadata compare operator got lost"));
				}
				break;
			}
			case analyzer::QueryTermExpression::Instruction::Operator:
			{
				if (isBooleanOp( ii->idx()))
				{
					...
					if (bop == MetaDataExpression::OperatorOR)
					{
						if (termc > 1)
						{
							...
							{
								throw strus::runtime_error(_TXT("CNF (conjunctive normal form) expected for meta data expression"));
							}
				...
				{
					throw strus::runtime_error(_TXT("internal: encountered illegal meta data operator, boolean operator expected as join of comparisons"));
				}
				break;
			}
		}
