...
{
	if (m_fieldno_stack.empty()) throw strus::runtime_error(_TXT("attach variable not allowed without any fields defined"));
	std::vector<unsigned int> fieldnoList( m_fieldno_stack.end()-1, m_fieldno_stack.end());
...
{
	if (m_fieldno_stack.empty()) throw strus::runtime_error(_TXT("assign feature not allowed without any fields defined"));
	std::vector<unsigned int> fieldnoList( m_fieldno_stack.end()-1, m_fieldno_stack.end());

	unsigned int groupid = newFeature( name, weight);
	QueryAnalyzerContextInterface::GroupBy groupBy = QueryAnalyzerContextInterface::GroupEvery;
	m_analyzer->groupElements( groupid, fieldnoList, groupBy, true/*groupSingle*/);
}
...
void TermExpression::translate( QueryInterface& query, const QueryProcessorInterface* queryproc)
{
	...
	if (nofargs)
	{
		throw strus::runtime_error(_TXT("not all term expressions assigned to features"));
	}
}
